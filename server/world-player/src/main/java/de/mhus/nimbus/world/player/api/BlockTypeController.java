...
public class BlockTypeController {
    ...
    @Operation(
            summary = "Get BlockTypes by group",
            description = "Returns all BlockTypes in a specific group for chunked loading")
    @ApiResponses({
    ...
    public ResponseEntity<?> getBlockTypesByGroup(HttpServletRequest request, @PathVariable String groupName) {
        ...
        if (!groupName.matches("^[a-z0-9_-]+$")) {
            return ResponseEntity.badRequest()
                    .body(Map.of(
                            "error",
                            "Invalid group name. Only lowercase letters, numbers, hyphens and underscores allowed."));
        }
    ...
    public ResponseEntity<?> getBlockType(HttpServletRequest request, @PathVariable String blockId) {
