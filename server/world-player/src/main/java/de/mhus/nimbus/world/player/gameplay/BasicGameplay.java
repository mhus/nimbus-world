...
public class BasicGameplay implements Gameplay {
    ...
    public void onBlockInteraction(
        ...
        if (!Strings.isBlank(shortcutKey)) {
            ...
            if (forceAction && blockAction != null && !blockAction.isBlank()) {
                ...
                boolean success = handler.handleBlockAction(
                        session, x, y, z, blockId, groupId, blockAction, params, userAction, shortcutKey, serverInfo);
                if (success) {
            ...
            if (!SHORTCUT_INTERACT_ACTION.equals(
                    itemAction)) { // in case of interact shortcut do the same as regular interaction, skip item action
                // routing
                if (!checkLogicCondition(session, serverInfo)) return;
                ...
                boolean success = handler.handleBlockAction(
                        session, x, y, z, blockId, groupId, itemAction, params, userAction, shortcutKey, serverInfo);
                if (success) {
                    sendItemUseFeedback(session, shortcutKey);
                    if (!logicHasPlaceholders(serverInfo)) fireLogicEffect(session, serverInfo, Map.of());
                }
        ...
        if (handler == null) {
            log.warn(
                    "Unknown block action '{}' in server metadata for block at {} ({}, {}, {})",
                    blockAction,
                    worldId,
                    x,
                    y,
        ...
        boolean success = handler.handleBlockAction(
                session, x, y, z, blockId, groupId, blockAction, params, userAction, null, serverInfo);
        if (success) {
    ...
    @Override
    public void onEntityInteraction(
            PlayerSession session,
            String entityId,
            String userAction,
            String shortcutKey,
        ...
        if ("entityProximity".equals(userAction)) {
            boolean entered = params != null
                    && params.has("entered")
                    && params.get("entered").asBoolean();
            if (entered) {
                String worldId =
                        session.getWorldId() != null ? session.getWorldId().getId() : null;
                if (worldId != null) {
        ...
        if (!Strings.isBlank(shortcutKey)) {
            ...
            if (!SHORTCUT_INTERACT_ACTION.equals(
                ...
                WEntity entity = entityService
                        .findByWorldIdAndName(session.getWorldId(), entityId, session.getEpoch())
                        .orElse(null);
                boolean success =
        ...
        if (worldIdStr != null && entityStateRedisService.isDead(worldIdStr, entityId)) {
            ...
            if (playerId != null && entityStateRedisService.removeLooter(worldIdStr, entityId, playerId)) {
                ...
                log.debug(
                        "Player {} is not eligible for loot from dead entity {} in world {}",
                        playerId,
                        entityId,
                        worldIdStr);
            }
        ...
        if (handler == null) {
            log.warn(
                    "Unknown entity action '{}' in server metadata for entity {} ({})",
                    entityAction,
                    session.getWorldId(),
                    entityId);
            return;
    ...
    protected String resolveShortcutItemAction(PlayerSession session, String shortcutKey, JsonNode params) {
        ...
        if (!"use".equals(shortcutDef.getType())) {
            log.warn(
                    "Unsupported shortcut type '{}' for shortcut '{}' in world {}",
                    shortcutDef.getType(),
                    shortcutKey,
                    session.getWorldId());
            return null;
    ...
    public void onItemInteraction(PlayerSession session, String itemId, JsonNode params) {
        ...
        if (handler == null) {
            log.warn(
                    "Unknown item action '{}' in server metadata for item {} ({})",
                    itemAction,
                    session.getWorldId(),
                    itemId);
            return;
    ...
    private void handleTeamMessage(PlayerSession session, JsonNode data) {
        ...
        String encodedMsg = I18nUtil.builder().en(msg).put("action", "message").build();
