...
class WSessionServiceTest {
    ...
    void createAndGetAndUpdateAndDelete() {
        ...
        Mockito.when(hashOps.entries(Mockito.matches("wsession.*")))
                .thenReturn(Map.of(
                        "status", "WAITING",
                        "world", "r1:w1",
                        "region", "r1",
        ...
        WSession session =
                svc.create(WorldId.of("r1:w1").get(), PlayerId.of("u1:c1").get(), "PLAYER");
    ...
    void cleanupExpiredRemovesOnlyExpired() {
        ...
        HashOperations hashOps = Mockito.mock(HashOperations.class);
        Mockito.doReturn(hashOps).when(template).opsForHash();
        Mockito.when(hashOps.get("wsession:session:expiredKey", "expire"))
                .thenReturn(Instant.now().minusSeconds(10).toString());
        Mockito.when(hashOps.get("wsession:session:validKey", "expire"))
                .thenReturn(Instant.now().plusSeconds(600).toString());
        ...
        Mockito.when(template.keys("wsession:session:*"))
                .thenReturn(java.util.Set.of("wsession:session:expiredKey", "wsession:session:validKey"));
