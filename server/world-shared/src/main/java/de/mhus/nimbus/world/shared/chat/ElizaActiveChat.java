...
public class ElizaActiveChat {
    ...
    private static final String[] GREETINGS = {
        "Hello! How are you feeling today?", "Hi there! What brings you here?", "Welcome! What's on your mind?"
    };
    ...
    private static final String[] THINKING_FILLERS = {
        "Hmm...", "Let me think about that...", "Interesting...", "One moment...", "Well...",
    };
    ...
    public List<WChatMessage> chat(WorldId worldId, String chatId, String playerId, String message) {
        log.debug(
                "Eliza processing: chatId={}, player={}, queue={}, message='{}'",
                chatId,
                playerId,
                queue != null ? "present" : "NULL",
                message);
        idleTicks = 0;
    ...
    private boolean matchesSleep(String lower) {
        return lower.contains("schlafe")
                || lower.contains("schlaf jetzt")
                || lower.contains("go to sleep")
                || lower.contains("sleep now")
                || lower.contains("pause")
                || lower.contains("ruh dich aus");
    }

    private boolean matchesArchive(String lower) {
        return lower.contains("es ist zuende")
                || lower.contains("es ist zu ende")
                || lower.contains("chat beenden")
                || lower.contains("tschüss")
                || lower.contains("auf wiedersehen")
                || lower.contains("goodbye")
                || lower.contains("end chat")
                || lower.contains("close chat")
                || lower.contains("it's over")
                || lower.contains("we're done");
    }
    ...
    private String generateResponse(String message) {
        ...
        if (lower.contains("feel")
                ...
                || lower.contains("worried")) return randomFrom(FEELING_RESPONSES);
        if (lower.matches("^(i am|i'm) .*")) return "Why are you " + lower.replaceFirst("^(i am|i'm) ", "") + "?";
