...
public class RUniverseClientService {
    ...
    public Optional<UniverseWorldDto> fetch(String regionId, String worldId) {
        String url = regionProperties.getUniverseBaseUrl() + "/universe/region/" + encode(regionId) + "/world/"
                + encode(worldId);
        try {
            var restTemplate =
                    RestTemplateUtil.create(jwtService.createTokenForSector(regionProperties.getSectorServerId()));
            @SuppressWarnings("unchecked")
            ResponseEntity<Map<String, Object>> resp =
                    (ResponseEntity) restTemplate.getForEntity(URI.create(url), Map.class);
            ...
            throw new RuntimeException("Universe world fetch failed: " + e.getMessage(), e);
        }
    }

    public UniverseWorldDto create(
            String regionId, String worldId, String name, String description, String coordinates) {
        ...
        try {
            ...
            var restTemplate =
                    RestTemplateUtil.create(jwtService.createTokenForSector(regionProperties.getSectorServerId()));
            @SuppressWarnings("unchecked")
            ResponseEntity<UniverseWorldDto> resp = restTemplate.postForEntity(url, request, UniverseWorldDto.class);
            if (resp.getStatusCode() == HttpStatus.CREATED && resp.getBody() != null) {
                return resp.getBody();
            }
            throw new IllegalStateException("Unexpected status " + resp.getStatusCode());
        } catch (RestClientException e) {
            throw new RuntimeException("Universe world create failed: " + e.getMessage(), e);
        }
    }

    public boolean createRegion(
            String token, String name, String apiUrl, FormattedKey publicSignKey, String maintainers) {
        if (name == null || name.isBlank()) throw new IllegalArgumentException("name blank");
        ...
        try {
            ...
            var restTemplate = RestTemplateUtil.create(
                    jwtService.createTokenForSectorServer(regionProperties.getSectorServerId()));
