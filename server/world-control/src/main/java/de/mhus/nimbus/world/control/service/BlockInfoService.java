...
import de.mhus.nimbus.world.shared.dto.BlockInfoDto;
import de.mhus.nimbus.world.shared.layer.WLayer;
import de.mhus.nimbus.world.shared.layer.WLayerService;
import de.mhus.nimbus.world.shared.redis.WorldRedisService;
import de.mhus.nimbus.world.shared.session.EditState;
import de.mhus.nimbus.world.shared.world.WChunkInfo;
...
public class BlockInfoService {
    ...
    /**
     * Load block info with layer metadata and WChunkInfo data.
     *
     ...
     * @return BlockInfoDto with block data, edit state, and chunk info metadata
     */
    @Transactional(readOnly = true)
    public BlockInfoDto loadBlockInfo(String worldId, String sessionId, int x, int y, int z) {
        ...
        // Load WChunkInfo data for this block
        String chunkInfoLayer = null;
        String chunkInfoGroup = null;
        try {
            ...
            if (world != null) {
                ...
                if (wid != null) {
                    Optional<WChunkInfo> infoOpt = chunkService.getChunkInfo(wid, chunkKey);
                    if (infoOpt.isPresent()) {
                        WChunkInfo info = infoOpt.get();
                        if (info.getBlockLayers() != null) {
                            chunkInfoLayer = info.getBlockLayers().get(coord);
                        }
                        if (info.getBlockGroups() != null) {
                            chunkInfoGroup = info.getBlockGroups().get(coord);
                        }
                    }
                }
            }
        } catch (Exception e) {
            log.debug("Could not load WChunkInfo for block ({},{},{}): {}", x, y, z, e.getMessage());
        }

        return new BlockInfoDto(block, readOnly, layerName, group, groupName, chunkInfoLayer, chunkInfoGroup);
    }
