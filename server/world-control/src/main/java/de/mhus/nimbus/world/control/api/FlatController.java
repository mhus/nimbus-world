...
public class FlatController extends BaseEditorController {
    ...
    public record FlatDetailDto(
            ...
            de.mhus.nimbus.generated.types.HexVector2 hexGrid,
            byte[] levels,
            byte[] columns,
            Instant createdAt,
            Instant updatedAt
    ...
    public ResponseEntity<byte[]> getHeightMap(
        ...
        try {
            // Generate height map image using helper class
            de.mhus.nimbus.world.shared.generator.FlatLevelImageCreator creator =
                    new de.mhus.nimbus.world.shared.generator.FlatLevelImageCreator(flat);
            byte[] imageBytes = creator.create(false); // ignoreEmptyMaterial = false for UI
    ...
    public ResponseEntity<byte[]> getBlockMap(
        ...
        try {
            // Generate block map image using helper class
            de.mhus.nimbus.world.shared.generator.FlatMaterialImageCreator creator =
                    new de.mhus.nimbus.world.shared.generator.FlatMaterialImageCreator(flat);
            byte[] imageBytes = creator.create(false); // ignoreEmptyMaterial = false for UI
