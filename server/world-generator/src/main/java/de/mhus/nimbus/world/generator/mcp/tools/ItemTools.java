...
public class ItemTools implements McpToolBean {
    ...
    @Tool(
            name = "list_items",
            description = "List all items for a region. Returns itemId, itemType, title, and enabled status.")
    public Map<String, Object> listItems(
            @ToolParam(description = "World ID or region (e.g. '@region:earth616')") String worldId,
            @ToolParam(description = "Optional search query to filter by title or itemType", required = false)
                    String query) {
        ...
        var dtos = entities.stream()
                .map(e -> {
                    ...
                    if (e.getPublicData() != null) {
                        map.put(
                                "itemType",
                                e.getPublicData().getItemType() != null
                                        ? e.getPublicData().getItemType()
                                        : "");
                        map.put(
                                "type",
                                e.getPublicData().getType() != null
                                        ? e.getPublicData().getType()
                                        : "");
                        map.put(
                                "title",
                                e.getPublicData().getTitle() != null
                                        ? e.getPublicData().getTitle()
                                        : "");
                        map.put(
                                "texture",
                                e.getPublicData().getTexture() != null
                                        ? e.getPublicData().getTexture()
                                        : "");
                    }
                    map.put("enabled", e.isEnabled());
                    if (e.getItemTier() != null)
                        map.put("itemTier", e.getItemTier().name());
                    if (e.getRarityCategory() != null)
                        map.put("rarityCategory", e.getRarityCategory().name());
    ...
    @Tool(
            name = "get_item",
            description = "Get a single item by its itemId. Returns full public data including modifier.")
    public Map<String, Object> getItem(
        ...
        if (entity.getItemTier() != null)
            result.put("itemTier", entity.getItemTier().name());
        if (entity.getRarityCategory() != null)
            result.put("rarityCategory", entity.getRarityCategory().name());
    ...
    @Tool(
            name = "create_item",
            description =
                    "Create or update an item. If no itemId is provided, one is auto-generated. All rendering properties (texture, scale, pose, etc.) are directly on the item.")
    public Map<String, Object> createItem(
            @ToolParam(description = "World ID or region (e.g. '@region:earth616')") String worldId,
            @ToolParam(description = "Optional item ID. If not provided, an ID is auto-generated.", required = false)
                    String itemId,
            ...
            @ToolParam(
                            description = "Player pose when holding (e.g. 'hold', 'eat', 'drink', 'sword', 'bow')",
                            required = false)
                    String pose,
            ...
            @ToolParam(description = "Whether item is generic/stackable (default false)", required = false)
                    Boolean generic,
            @ToolParam(description = "Additional parameters as key-value pairs", required = false)
                    Map<String, String> parameters,
            @ToolParam(description = "Server-side parameters for gameplay configuration", required = false)
    ...
    @Tool(
            name = "duplicate_item",
            description =
                    "Duplicate an existing item. Creates a new item with a new auto-generated itemId, copying all properties from the source item. Optionally override the title.")
    public Map<String, Object> duplicateItem(
            @ToolParam(description = "World ID or region (e.g. '@region:earth616')") String worldId,
            @ToolParam(description = "Item ID of the existing item to duplicate") String itemId,
            @ToolParam(
                            description = "Optional new title for the duplicate. If blank, keeps the original title.",
                            required = false)
                    String title) {
    ...
    @Tool(
            name = "update_items_by_type",
            description =
                    "Batch update parameters for all items matching a specific itemType. Merges the given parameters into each item's existing parameters.")
    ...
    @Tool(
            name = "update_item",
            description =
                    "Update parameters, server properties, and/or trading fields on an existing item by itemId. Merges the given values into the item's existing maps. Trading fields (itemTier, rarityCategory, basePrice, etc.) are set directly.")
    public Map<String, Object> updateItem(
            @ToolParam(description = "World ID or region (e.g. '@region:earth616')") String worldId,
            @ToolParam(description = "Item ID to update") String itemId,
            @ToolParam(description = "Parameters to merge into the item's public parameters", required = false)
                    Map<String, String> parameters,
            @ToolParam(
                            description =
                                    "Server-side parameters to merge into the item's server map (e.g. action, effects)",
                            required = false)
                    Map<String, String> server,
            @ToolParam(
                            description =
                                    "Item tier: NONE, LEATHER, IRON, STEEL, SILVER, GOLD, MYTHRIL, ADAMANT, ORICHALCUM",
                            required = false)
                    String itemTier,
            @ToolParam(description = "Rarity: COMMON, UNCOMMON, RARE, EPIC, LEGENDARY, MYTHIC", required = false)
                    String rarityCategory,
            @ToolParam(description = "Base price in silver (overrides calculated value)", required = false)
                    Double basePrice,
            ...
            @ToolParam(description = "Usage bonus component (combat stats, utility)", required = false)
                    Double usageBonus,
    ...
    @Tool(
            name = "rename_item",
            description =
                    "Rename the itemId of an existing item. The new itemId must not already exist in the same world.")
