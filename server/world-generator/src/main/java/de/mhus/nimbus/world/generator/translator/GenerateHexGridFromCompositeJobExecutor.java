...
public class GenerateHexGridFromCompositeJobExecutor implements JobExecutor {
    ...
    public JobResult execute(WJob job) throws JobExecutionException {
        try {
            ...
            log.info(
                    "Generating hexgrids: documentId={}, epoch={}, seed={}, edge_blend_width={}, edge_blend_randomness={}, edge_shake_strength={}, edge_blur_radius={}",
                    documentId,
                    epoch,
                    seed,
                    edgeBlendWidth,
            ...
            log.info(
                    "Loaded enriched composition: name='{}', worldId='{}', featureHexGrids={}",
                    composition.getName(),
            ...
            if (composition.getFeatureHexGridRegistry() == null
                ...
                log.error(
                        "No FeatureHexGrids found in Central Registry. This composition was likely created with an older "
            ...
            for (var featureHexGrid : composition.getFeatureHexGridRegistry().values()) {
                ...
                de.mhus.nimbus.world.shared.world.WHexGrid sourceGrid =
                        createWHexGridFromFeatureHexGrid(featureHexGrid, composition.getWorldId());
                ...
                var existingList = hexGridService.findAllByWorldIdAndPosition(composition.getWorldId(), coord);
                ...
                if (!existingList.isEmpty()) {
                    ...
                    if (sourceGrid.getPublicData() != null) {
                        ...
                        if (sourceGrid.getPublicData().getName() != null) {
                            wHexGrid.getPublicData()
                                    .setName(sourceGrid.getPublicData().getName());
                        }
                        if (sourceGrid.getPublicData().getTitle() != null) {
                            wHexGrid.getPublicData()
                                    .setTitle(sourceGrid.getPublicData().getTitle());
                        }
                        if (sourceGrid.getPublicData().getDescription() != null) {
                            wHexGrid.getPublicData()
                                    .setDescription(sourceGrid.getPublicData().getDescription());
                        }
                    }
                    ...
                    log.debug(
                            "Updated WHexGrid: {} at {} with {} parameters, name={}, title={}",
                            composition.getWorldId(),
                            position,
                            params.size(),
                            wHexGrid.getPublicData() != null
                                    ? wHexGrid.getPublicData().getName()
                                    : null,
                            wHexGrid.getPublicData() != null
                                    ? wHexGrid.getPublicData().getTitle()
                                    : null);
                    updatedGrids++;
                } else {
                    ...
                    log.debug(
                            "Created WHexGrid: {} at {} with {} parameters, name={}, title={}",
                            composition.getWorldId(),
                            position,
                            params.size(),
                            publicData.getName(),
            ...
            log.info(
                    "WHexGrid creation/update complete: created={}, updated={}, total={}",
                    createdGrids,
                    updatedGrids,
                    composition.getFeatureHexGridRegistry().size());
            ...
            for (String position : allCoordinateStrings) {
                ...
                var gridList = hexGridService.findAllByWorldIdAndPosition(composition.getWorldId(), coord);
                ...
                //                for (de.mhus.nimbus.world.shared.world.WHexGrid.EDGE side :
                ...
                //                        String neighborFlatId = "genesis_" + neighborCoord.getQ() + "_" +
                // neighborCoord.getR();
            ...
            String coordinatesStr = allCoordinateStrings.stream().sorted().collect(Collectors.joining(" "));
            ...
            log.info(
                    "GenerateHexGridFromComposite completed: gridCount={}, created={}, updated={}",
                    composition.getFeatureHexGridRegistry().size(),
                    createdGrids,
                    updatedGrids);
    ...
    private WDocument loadDocumentFromPath(String worldId, String documentId) {
        ...
        try {
            ...
            if (documentOpt.isEmpty()) {
                log.warn(
                        "Document not found: worldId={}, collection={}, id={}",
                        worldId,
                        COMPOSED_COLLECTION,
                        documentId);
                return null;
    ...
    private de.mhus.nimbus.world.shared.world.WHexGrid createWHexGridFromFeatureHexGrid(
            de.mhus.nimbus.world.generator.composer.feature.FeatureHexGrid featureHexGrid, String worldId) {
