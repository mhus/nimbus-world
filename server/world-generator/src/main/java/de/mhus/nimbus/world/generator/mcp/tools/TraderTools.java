...
public class TraderTools implements McpToolBean {
    ...
    @Tool(
            name = "list_traders",
            description = "List all traders for a world. Returns entityId, traderType, categories, silverAmount.")
    ...
    @Tool(
            name = "create_trader",
            description =
                    "Create or update a trader linked to an NPC entity. Requires entityId, worldId, and at least chestId. The trader type defaults to MERCHANT.")
    public Map<String, Object> createTrader(
            ...
            @ToolParam(description = "Item categories the trader handles (e.g. ['food', 'material'])", required = false)
                    List<String> categories,
            @ToolParam(
                            description = "Personality price modifier (positive=expensive, negative=cheap)",
                            required = false)
            ...
            @ToolParam(description = "Max items displayed to player (default 12)", required = false)
                    Integer maxDisplayItems,
            @ToolParam(description = "Gold to silver exchange rate (default 10)", required = false)
                    Double goldExchangeRate,
            @ToolParam(description = "Pool sync interval in seconds (default 3600)", required = false)
        ...
        if (existing.isPresent()) {
            ...
            trader = WTrader.builder().worldId(worldId).entityId(entityId).build();
        }
