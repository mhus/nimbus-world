...
public class StructuresGeneratorService {
    ...
    public StructuresIndex findStructuresForWorldId(String worldId) {
        ...
        if (layerOpt.isEmpty()) {
            log.info(
                    "No '{}' layer found in region collection {} for worldId={}",
        ...
        for (WLayerModel model : modelSummaries) {
            ...
            if (Strings.isBlank(kind) || Strings.isBlank(style)) {
                log.debug(
                        "Skipping model '{}' - missing kind or style (kind={}, style={})",
                        model.getName(),
                        kind,
                        style);
                skipped++;
        ...
        log.info(
                "StructuresIndex for worldId={}: loaded {} buildings, skipped {} (total models: {})",
                worldId,
                loaded,
                skipped,
                modelSummaries.size());
    ...
    public int clearStructuresInHexGrid(HexVector2 coordinate, WLayer layer, WWorld world) {
        ...
        for (WLayerModel model : models) {
            if (HexMathUtil.isPointInHex(model.getMountX(), model.getMountZ(), hexCenterX, hexCenterZ, gridSize)) {
                ...
                deleted++;
                log.debug(
                        "Deleted structure model '{}' (mount={},{}) from hex {}",
                        model.getName(),
                        model.getMountX(),
                        model.getMountZ(),
                        coordinate);
        ...
        log.info(
                "Cleared {} structure models in hex ({},{}) for layer {}",
                deleted,
                coordinate.getQ(),
                coordinate.getR(),
                layer.getName());
    ...
    public void syncLayerModelsToTerrain(WWorld world, WLayer layer, HexVector2 coordinate) {
        ...
        log.info(
                "Syncing structure models to terrain for hex ({},{}) in layer {}",
                coordinate.getQ(),
                coordinate.getR(),
                layer.getName());
        ...
        log.info(
                "Synced structure models to terrain: {} chunks recreated for layer {} in hex ({},{})",
                chunks,
                layer.getName(),
                coordinate.getQ(),
                coordinate.getR());
