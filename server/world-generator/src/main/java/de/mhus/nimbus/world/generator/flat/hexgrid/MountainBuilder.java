...
public class MountainBuilder extends HexGridBuilder {
    ...
    private static final int RIDGE_WIDTH = 15;  // Width of ridge effect in pixels
    ...
    public void buildFlat() {
        ...
        applyRidgeTransformations(flat, baseHeight, hillHeight);
     ...
     */
    private void applyRidgeTransformations(WFlat flat, int baseHeight, int hillHeight) {
        ...
        try {
            ...
            for (RidgeDefinition ridge : ridges) {
                applyRidgeTransformation(flat, ridge, baseHeight, hillHeight);
            }
     ...
     */
    private void applyRidgeTransformation(WFlat flat, RidgeDefinition ridge, int baseHeight, int hillHeight) {
        log.debug("Applying ridge transformation: side={}, level={}", ridge.getSide(), ridge.getLevel());
        ...
        for (int z = 0; z < sizeZ; z++) {
            for (int x = 0; x < sizeX; x++) {
                ...
                if (distanceToRidge < RIDGE_WIDTH) {
                    int currentLevel = flat.getLevel(x, z);

                    // Calculate ridge factor (1.0 at ridge line, 0.0 at RIDGE_WIDTH distance)
                    double ridgeFactor = 1.0 - (distanceToRidge / RIDGE_WIDTH);
                    ...
                    int newLevel = (int) (currentLevel + ridgeFactor * (ridgeHeight - currentLevel));
