...
public class OceanEdgePoint extends Point {
    ...
    public de.mhus.nimbus.generated.types.HexVector2 selectGridCoordinate(Area biome, ComposeContext context) {
        ...
        log.info(
                "OceanEdgePoint {}: Biome {} (center={}) has {} coordinates",
                getName(),
                biome.getName(),
                biome.getPlacedCenter(),
                coordinates.size());
        ...
        log.info(
                "OceanEdgePoint {}: Found {} coords in biome, also checking adjacent filler grids",
                getName(),
                edgeCoords.size());
        List<de.mhus.nimbus.generated.types.HexVector2> adjacentFillers =
                findAdjacentFillerGridsWithOcean(biome, context);
        if (!adjacentFillers.isEmpty()) {
            log.info(
                    "OceanEdgePoint {}: Found {} adjacent filler grids with ocean access: {}",
                    getName(),
                    adjacentFillers.size(),
                    adjacentFillers.stream()
                            .limit(10)
                            .map(c -> String.format("(%d,%d)", c.getQ(), c.getR()))
                            .toList());
        ...
        if (oceanDirection != null && biome.getPlacedCenter() != null) {
            ...
            log.info(
                    "OceanEdgePoint {}: Selected grid ({},{}) in direction {}",
                    getName(),
                    selected.getQ(),
                    selected.getR(),
                    oceanDirection);
            ...
            log.warn(
                    "OceanEdgePoint {}: No oceanDirection set, using first coordinate ({},{})",
                    getName(),
                    selected.getQ(),
                    selected.getR());
        }
    ...
    public de.mhus.nimbus.world.shared.world.HexLocalEdgeVector composePosition(Area biome, ComposeContext context) {
        ...
        if (oceanEdge == null) {
            log.warn(
                    "OceanEdgePoint {}: No ocean/coast neighbor at grid ({},{}), using fallback",
                    getName(),
                    gridCoord.getQ(),
                    gridCoord.getR());
            oceanEdge = oceanDirection != null
                    ? directionToEdge(oceanDirection)
                    : de.mhus.nimbus.world.shared.world.WHexGrid.EDGE.WEST;
        }
        ...
        log.info(
                "OceanEdgePoint {}: Placed at grid ({},{}) edge {} leading to ocean/coast",
                getName(),
                gridCoord.getQ(),
                gridCoord.getR(),
                oceanEdge);
     ...
     */
    private de.mhus.nimbus.world.shared.world.WHexGrid.EDGE findOceanEdge(
            de.mhus.nimbus.generated.types.HexVector2 coord, ComposeContext context) {
        log.info(
                "OceanEdgePoint {}: Checking neighbors of grid ({},{}) to find ocean/coast edge",
                getName(),
                coord.getQ(),
                coord.getR());
        ...
        for (de.mhus.nimbus.world.shared.world.WHexGrid.EDGE edge :
            ...
            if (neighborBiome == null) {
                ...
                log.info(
                        "  -> Edge {}: neighbor ({},{}) is {} (biome: {})",
                        edge,
                        neighborCoord.getQ(),
                        neighborCoord.getR(),
                        type,
                        neighborBiome.getName());

                if (type == BiomeType.OCEAN || type == BiomeType.COAST) {
                    log.info(
                            "*** OceanEdgePoint {}: FOUND ocean/coast at edge {} pointing to ({},{}) type={}",
                            getName(),
                            edge,
                            neighborCoord.getQ(),
                            neighborCoord.getR(),
                ...
                log.info(
                        "  -> Edge {}: neighbor ({},{}) is not a Biome ({})",
                        edge,
                        neighborCoord.getQ(),
                        neighborCoord.getR(),
                        neighborBiome.getClass().getSimpleName());
            }
        }

        log.warn(
                "OceanEdgePoint {}: No ocean/coast neighbor found for grid ({},{})",
                getName(),
                coord.getQ(),
                coord.getR());
        return null;
     ...
     */
    private List<de.mhus.nimbus.generated.types.HexVector2> findAdjacentFillerGridsWithOcean(
            Area biome, ComposeContext context) {
    ...
    private Area getBiomeAt(de.mhus.nimbus.generated.types.HexVector2 coord, ComposeContext context) {
        for (PlacedBiome placed : context.getPlacedBiomes()) {
            if (placed.getCoordinates().stream().anyMatch(c -> c.getQ() == coord.getQ() && c.getR() == coord.getR())) {
    ...
    private double scoreCoordInDirection(
