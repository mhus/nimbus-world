...
public class FlatExportService {
    ...
    public int exportToLayer(String flatId, String worldId, String layerName, boolean smoothCorners) {
        ...
        for (int localX = 0; localX < flat.getSizeX(); localX++) {
            for (int localZ = 0; localZ < flat.getSizeZ(); localZ++) {
                ...
                LayerChunkData layerChunkData = modifiedChunks.computeIfAbsent(chunkKey, key -> {
     ...
     */
    private String getBlockDefAtPosition(ChunkData chunkData, int worldX, int worldZ, int y) {
    ...
    /**
     * Find highest block Y at a specific X,Z position, resolving across chunk boundaries.
     * Determines the correct chunk for the given world coordinates and looks up blocks there.
     *
    ...
    private int findHighestBlockAtPositionCrossChunk(int worldX, int worldZ, WWorld world,
        ...
        return findHighestBlockAtPosition(chunkDataOpt.get(), worldX, worldZ);
    }
     ...
     * @return Highest Y coordinate, or -1 if no block found
     */
    private int findHighestBlockAtPosition(ChunkData chunkData, int worldX, int worldZ) {
        int highestY = -1;
     ...
     */
    private int findHighestGroundBlockAtPosition(ChunkData chunkData, int worldX, int worldZ, String worldId) {
    ...
    private void fillColumn(LayerChunkData chunkData, int worldX, int worldZ,
        ...
        for (int y = 255; y >= lowestY; y--) {
            ...
            if (isExtra) {
                ...
                groupId = flat.getGroup(localX, y, localZ);
            } else if (y == level) {
                // Top block (y == level): lookup groupId with x/z
                groupId = flat.getGroup(localX, localZ);
            }
