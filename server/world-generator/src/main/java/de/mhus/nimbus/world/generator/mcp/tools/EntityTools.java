...
@Slf4j
public class EntityTools implements McpToolBean {
    ...
    @Tool(
            name = "list_entities",
            description =
                    "List all entities for a world. Returns entityId, modelId, name, enabled status, position, epoches, and server parameters. Use epoch parameter to filter by specific epoch.")
    public Map<String, Object> listEntities(
            @ToolParam(description = "World ID (e.g. 'ymir:Mist'). Must be a world ID, not a collection.")
                    String worldId,
            @ToolParam(description = "Optional search query to filter by entityId or name", required = false)
                    String query,
            @ToolParam(
                            description = "Optional epoch number to filter entities belonging to this epoch",
                            required = false)
                    Integer epoch) {
    ...
    @Tool(
            name = "get_entity",
            description =
                    "Get a single entity by its entityId. Returns full data including publicData, position, behavior, and server parameters.")
    public Map<String, Object> getEntity(
        ...
        WEntity entity = entityService
                .findByWorldIdAndName(wid, entityId)
    ...
    @Tool(
            name = "create_entity",
            description =
                    "Create a new entity in the world. Entities are instances of EntityModels placed in the world (e.g., NPCs, creatures).")
    public Map<String, Object> createEntity(
            ...
            @ToolParam(description = "Movement type: 'static', 'passive', 'slow', 'dynamic'", required = false)
                    String movementType,
            @ToolParam(description = "Controlled by: 'player', 'server', 'ai', 'client'", required = false)
                    String controlledBy,
            @ToolParam(description = "Whether the entity is solid (collision)", required = false) Boolean solid,
            @ToolParam(description = "Whether the entity is interactive (can be clicked)", required = false)
                    Boolean interactive,
            ...
            @ToolParam(description = "Behavior model identifier (e.g. 'PreyAnimalBehavior')", required = false)
                    String behaviorModel,
            @ToolParam(
                            description = "Server-side parameters as key-value pairs for gameplay configuration",
                            required = false)
                    Map<String, String> server,
            @ToolParam(
                            description =
                                    "Epoch numbers this entity belongs to (e.g. [0,1,2]). If not specified, defaults to empty list (= not visible in any epoch).",
                            required = false)
                    List<Integer> epoches,
            @ToolParam(
                            description = "Entity type: OTHER, ANIMAL, NPC, PLAYER, REMOTE. Default: OTHER",
                            required = false)
                    String entityType,
            @ToolParam(description = "Path to portrait image (e.g. for dialog UI)", required = false)
                    String portraitPath,
            @ToolParam(description = "Source/generator identifier (e.g. 'npc-generator')", required = false)
                    String source) {
    ...
    @Tool(
            name = "update_entity",
            description = "Update an existing entity. Only provided fields are updated, others remain unchanged.")
    public Map<String, Object> updateEntity(
            ...
            @ToolParam(
                            description = "Server-side parameters to merge into existing server parameters",
                            required = false)
                    Map<String, String> server,
            @ToolParam(
                            description = "Epoch numbers this entity belongs to (replaces existing epoches)",
                            required = false)
                    List<Integer> epoches,
            @ToolParam(description = "Entity type: OTHER, ANIMAL, NPC, PLAYER, REMOTE", required = false)
                    String entityType,
    ...
    public Map<String, Object> deleteEntity(
        ...
        return Map.of("deleted", true, "entityId", entityId);
    }
}
