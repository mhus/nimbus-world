 ...
 */
@Slf4j
public class TownExternalConnectionGenerator {
    ...
    public GenerationResult generateExternalConnections(HexComposition composition,
        ...
        for (Feature feature : composition.getFeatures()) {
            if (feature instanceof Town) {
                villages.add((Town) feature);
            }
        }
        ...
        for (Town village : villages) {
            List<TownConnectionPoint> villagePoints = generateForVillage(village, placementResult);
     ...
     */
    private HexVector2 getVillageCenter(Town village) {
     ...
     */
    private boolean isGridSuitable(HexVector2 gridCoord, Town village) {
     ...
     */
    private Map<Direction, String> findInternalConnectionPoints(Town village) {
    ...
    public static class GenerationResult {
        private final List<TownConnectionPoint> points = new ArrayList<>();

        public void addPoints(List<TownConnectionPoint> newPoints) {
            points.addAll(newPoints);
        }

        public List<TownConnectionPoint> getPoints() {
            return points;
        }
