        ...
        <dialog ref="importDialogRef" class="modal">
          <div class="modal-box max-w-2xl">
            ...
            <p class="py-4 text-sm text-base-content/70">
              Select a .model.json file generated by schematic-tool to import.
            </p>
            ...
            <div class="form-control">
              ...
              <input
                ...
                :class="{ 'file-input-error': importError }"
                @change="handleFileSelected"
              />
              <label v-if="selectedFileName" class="label">
                <span class="label-text-alt">Selected: {{ selectedFileName }}</span>
              </label>
...
const handleFileSelected = async (event: Event) => {
  ...
  try {
    ...
    importJsonData.value = text;
    logger.debug('File loaded', { fileName: file.name, size: text.length });
  } catch (error: any) {
    logger.error('Failed to read file', { fileName: file.name }, error);
    importError.value = `Failed to read file: ${error.message}`;
    importJsonData.value = '';
  }
};
