      ...
      <tbody>
        <template v-for="job in jobs" :key="job.id">
          ...
          <tr
            ...
            class="border-b-0 cursor-pointer hover:bg-base-300"
            @click="$emit('view', job)"
          >
            <td colspan="11" class="py-2">
              <div class="font-semibold text-base flex items-center gap-2">
                <!-- Parent indicator -->
                <svg v-if="job.parent" class="w-4 h-4 text-info flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" title="Child Job">
          ...
          <tr
            :class="{ 'bg-base-200/50': job.parent }"
            class="border-t-0 cursor-pointer hover:bg-base-300"
            @click="$emit('view', job)"
          >
            <!-- ID -->
            <td>
            ...
            <td class="w-24">
              <div v-if="job.errorMessage" class="text-xs text-error max-w-xs truncate">
                {{ job.errorMessage }}
              </div>
            </td>
