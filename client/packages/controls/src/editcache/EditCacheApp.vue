<template>
  <div class="min-h-screen flex flex-col">
    ...
    <header class="navbar bg-base-300 shadow-lg">
      ...
      <div class="flex-1">
        <h1 class="text-xl font-bold px-4">Nimbus Edit Cache Editor</h1>
      </div>
    ...
    <main class="flex-1 container mx-auto px-4 py-6">
      ...
      <div v-if="!effectiveWorldId" class="alert alert-info">
                ...
                <table class="table table-zebra w-full">
                  <thead>
                    <tr>
                      <th>Layer Name</th>
                      <th>Block Count</th>
                      <th v-if="selectedEpoch === -1">Epochs</th>
                      <th>First Edit</th>
                      <th>Last Edit</th>
                      <th>Actions</th>
                    </tr>
                  ...
                  <tbody>
                    <tr v-for="stat in statistics" :key="stat.layerDataId">
                      ...
                      <td v-if="selectedEpoch === -1" class="text-sm">
                        <span
                          v-for="(count, epoch) in stat.epochBreakdown"
                          :key="epoch"
...
interface EditCacheStat {
  ...
  epochBreakdown?: Record<string, number>;
}
...
const loadEpoches = async () => {
  ...
  try {
    ...
    if (epoches.value.length > 0) {
      selectedEpoch.value = -1; // Default: show all epochs
    }
  } catch {
    epoches.value = [];
  }
...
// Build the full worldId with epoch suffix, or base worldId for "All"
const effectiveWorldId = computed(() => {
  if (!currentWorldId.value) return '';
  if (epoches.value.length === 0) return currentWorldId.value;
  if (selectedEpoch.value === -1) return currentWorldId.value; // All epochs: base worldId
...
const loadStatistics = async (silent = false) => {
  ...
  try {
    const response = await axios.get(
      `${apiUrl.value}/control/editor/${effectiveWorldId.value}/editcache/statistics`,
...
const handleApplyChanges = async (layerDataId: string, layerName: string) => {
  ...
  try {
    await axios.post(
      `${apiUrl.value}/control/editor/${effectiveWorldId.value}/editcache/${layerDataId}/apply`,
      {},
      { withCredentials: true }
    );
...
const handleDiscardChanges = async (layerDataId: string, layerName: string) => {
  ...
  try {
    const response = await axios.post(
      `${apiUrl.value}/control/editor/${effectiveWorldId.value}/editcache/${layerDataId}/discard`,
      {},
      { withCredentials: true }
    );
...
// Watch for world changes: reload epoches
watch(currentWorldId, () => {
  loadEpoches();
}, { immediate: true });

// Watch for effective worldId changes (world or epoch): reload statistics
